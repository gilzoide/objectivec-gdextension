...
func _on_alert_button_pressed():
	match OS.get_name():
		"macOS":
			var alert = NSClass.from_string("NSAlert").alloc("init")
			alert.messageText = ALERT_TITLE
			alert.informativeText = ALERT_BODY
			alert.perform_selector("runModal")
		"iOS":
			var UIAlertController = NSClass.from_string("UIAlertController")
			var UIAlertAction = NSClass.from_string("UIAlertAction")
			...
			var view_controller = NSClass.from_string("UIApplication").sharedApplication.delegate.window.rootViewController
