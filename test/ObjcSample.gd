...
func _on_alert_button_pressed():
	match OS.get_name():
		"macOS":
			var alert = ObjectiveC.get_class("NSAlert").alloc("init")
		...
		"iOS":
			...
			var view_controller = ObjectiveC.get_class("UIApplication").sharedApplication.delegate.window.rootViewController
