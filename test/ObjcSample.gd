...
func _on_alert_button_pressed():
	match OS.get_name():
		"macOS":
			var alert = ObjectiveCClass.from_string("NSAlert").alloc("init")
			alert.messageText = ALERT_TITLE
			alert.informativeText = ALERT_BODY
			alert.perform_selector("runModal")
		"iOS":
			var UIAlertController = ObjectiveCClass.from_string("UIAlertController")
			var UIAlertAction = ObjectiveCClass.from_string("UIAlertAction")
			...
			var view_controller = ObjectiveCClass.from_string("UIApplication").sharedApplication.delegate.window.rootViewController
